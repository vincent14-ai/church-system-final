...
// added church_ministry, spiritual_trainings, water_baptized,
export async function getMembers(filters = {}) {
  const {
    search, gender, marital_status, age_group,
    member_status, date_from, date_to, church_ministry, spiritual_trainings,
    water_baptized,
  } = filters;
  ...
  if (
  spiritual_trainings &&
  Array.isArray(spiritual_trainings) &&
  spiritual_trainings.length > 0
) {
  query = query.in(
    'spiritual_trainings.training_type',
    spiritual_trainings
  );
}
  ...
  if (date_from) {
    query = query.gte('date_attended', date_from);
  }

  if (date_to) {
    query = query.lte('date_attended', date_to);
  }
