...
export const Reports = ({ isDark, onToggleTheme }) => {
    ...
    const [members, setMembers] = useState(HARDCODED_MEMBERS); 
...
    const [selectedMaritalStatus, setSelectedMaritalStatus] = useState("all"); 
    ...
    const [attendanceRecords, setAttendanceRecords] = useState(HARDCODED_ATTENDANCE);
    ...
    const fetchMembers = async () => {
        setIsLoading(true);
        try {
            const res = await API.get("http://localhost:5000/api/members", { 
                params: {
                    search: searchTerm,
...
                    marital_status: selectedMaritalStatus,
    ...
    const fetchAttendance = async () => {
        setIsLoading(true);
        try {
            const res = await API.get("http://localhost:5000/api/attendance", { 
                params: {
                    date: attendanceDate,
...
        setSelectedMaritalStatus("all"); 
    ...
    const handleEdit = async (member_id) => {
        setIsLoading(true);
        try {
            const res = await API.getById(member_id); 
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            ...
            await API.put(selectedMember, payload); 
    ...
    const handleDelete = async (member_id) => {
        const confirmed = window.confirm("Are you sure you want to permanently delete this member?");
        if (!confirmed) return;

        try {
            const res = await API.delete(member_id); 
    ...
    const AttendanceRecords = () => (
        <>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pt-4 pb-6 border-b border-gray-200 dark:border-gray-700 mb-6">
                ...
                <Button variant="outline" className="flex items-center gap-2 border-teal-400 text-teal-600 dark:border-teal-600 dark:text-teal-400 hover:bg-teal-50 dark:hover:bg-teal-900/50 transition-colors">
                        ...
                        {isLoading ? (
                            ...
                            currentAttendanceRows.map((record) => (
                                <TableRow key={record.attendance_id} className="text-gray-700 dark:text-gray-300 hover:bg-teal-50/50 dark:hover:bg-gray-800/70 border-b border-gray-100 dark:border-gray-700/50 transition-colors">
                                    <TableCell className="font-mono text-xs">{record.member_id}</TableCell>
